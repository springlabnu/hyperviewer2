function writeBasisAsTiffStack(im, save_dir, subdirs, app)
...
imwrite(colorcomp, [save_dir subdirs{1} im.filename 'basis_composite_image.tif']);

% Set up tiff objects for writing stacks

tColor = Tiff([save_dir subdirs{2} im.filename 'basis_stack_color.tif'],'w');
tBW = Tiff([save_dir subdirs{3} im.filename 'basis_stack_intensity_map.tif'],'w');
